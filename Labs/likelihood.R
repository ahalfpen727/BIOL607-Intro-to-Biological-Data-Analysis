...
dpois(10, lambda = 12)
log(dpois(10, lambda = 12))
#--------------
#for grid sampling, we search lambda ####
#--------------
pois_10 <- dpois(x = 10, lambda = 0:20)
pois_10
idx_mle <- which(pois_10 == max(pois_10))
idx_mle
...
pois_10 <- data.frame(lambda = 0:20)
pois_10$lik = dpois(10, lambda = pois_10$lambda)
pois_10$ll = dpois(10, lambda = pois_10$lambda, log=T)

#plot
ggplot(pois_10,aes(x = lambda, y = lik))+
   geom_point()
ggplot(pois_10,aes(x = lambda, y = ll))+
   geom_point()
...
#get the MLE
pois_10 %>% filter(lik == max(lik)) -> pois_10.mle
pois_10.mle
#95% CI from a chisq?
qchisq(0.95, df = 1)/2
...
which(pois_10$ll >= max(pois_10$ll)-1.92)
...
my_lik_fun <- function(observations, predictors,  parameters)

   #1. A data generating process
  # takes predictors and parameters
  # and calculates fitted values of observation

    #2. A likelihood function
  # compares fitted and observed values
  # and gets the likelihood parameters | observations

# example for a poisson log likelihood
pois.lik <- function(y, lambda_val){
   #Data Generating Process
   y_hat <- lambda_val
   #LogLik Function
   #use our observations (y) and fitted values (y_hat)
   sum(dpois(y, lambda = y_hat))}

pois.loglik <- function(y, lambda_val){
  #Data Generating Process
  y_hat <- lambda_val
  #LogLik Function
  #use our observations (y) and fitted values (y_hat)
...
pois_data
pois.loglik(pois_data, lambda_val = 3)
pois.lik(pois_data, lambda_val = 3)

#grid sample
pois_mle <- data.frame(lambda_val = 0:20) %>%
  rowwise() %>%
  mutate(ll = pois.loglik(pois_data, lambda_val)) %>%
  ungroup()
#grid sample
pois_mle <- data.frame(lambda_val = 0:20) %>%
   rowwise() %>%
   mutate(ll = pois.lik(pois_data, lambda_val)) %>%
   ungroup()
...
sum(dbinom(x = c(11, 10, 8, 9, 7), size = 20, prob = 0.5, log = TRUE))
