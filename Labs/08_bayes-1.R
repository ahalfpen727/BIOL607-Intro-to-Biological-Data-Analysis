...
puffer <- read.csv("http://www.zoology.ubc.ca/~whitlock/ABD/teaching/datasets/17/17q11PufferfishMimicry%20Caley%20&%20Schluter%202003.csv")

parallel::detectCores()
options(mc.cores = parallel::detectCores())

seal_lm_bayes<-brm(length ~ ageInDays, family = gaussian(link = "identity"), data = seals, file = "./seal_lm_bayes.brms")
